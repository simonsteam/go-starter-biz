...
type User struct {
	ID          uint32    `json:"id"`
	BranchID    int64     `json:"branchId"`                          // 分部Id，除系统管理角色外的账号必须指定门店
	...
	AddTime     time.Time `json:"addTime,omitempty"`                 // 添加时间
	Groups      []Group   `json:"groups" pg:"many2many:user_groups"` // 职位/角色 列表
}
...
// BeforeInsert go-pg hook
func (u *User) BeforeInsert(orm.DB) error {
	u.AddTime = time.Now()
	return nil
}
